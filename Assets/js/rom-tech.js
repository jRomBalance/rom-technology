...
(function () {
  ...
  function initLogoSwitch() {
    const heroLogo = document.querySelector('.rom-hero-logo');
    if (!heroLogo) return;

    const SWITCH_THRESHOLD = window.innerHeight * 0.6;

    // Use favicon-512 as the small version
    const masterSrc  = heroLogo.src;
    const faviconSrc = (window.romTheme && window.romTheme.faviconUrl)
      ? window.romTheme.faviconUrl
      : masterSrc;
    ...
    function updateLogo() {
      ...
      if (scrolled && !isSmall) {
        isSmall = true;
        heroLogo.style.transition = 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)';
        heroLogo.style.width      = '80px';
        heroLogo.style.opacity    = '0.7';
